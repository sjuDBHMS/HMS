...
<body>
  ...
  <?php
	  if (isset($_REQUEST['deletePatient'])){
		...
		$phoneDeleteQuery = "DELETE FROM Patient_Phone WHERE PatientID='$pid'";
		mysql_query($phoneDeleteQuery);
...
/*
 			$query="select * FROM Patient where DOB between (select DOB from Patient where DOB like '%$dateFrom%' limit 1)
and (select DOB from Patient where DOB like '%$dateTo%' limit 1)";
*/
			$query="select * FROM Patient where DOB between '$dateFrom' and '$dateTo'";
			$results = mysql_query($query);
			echo($res);
